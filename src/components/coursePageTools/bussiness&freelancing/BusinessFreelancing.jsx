...
import '../featuredCard/featured.css'
import { Link } from 'react-router-dom';
import { CartState } from '../../../context/Context';

const BusinessFreelancing = (props) => {
  const { data } = props;
          ...
          if (e.type === 'BS') {
            return (
              <SwiperSlide key={e.id}>
                <div className="relative aspect-[4/3] md:max-w-sm bg-white rounded-lg border-2 border-white shadow-md  duration-500  mb-6 md:mb-0 w-full h-[60vh]">
                  ...
                  <div className="p-5">
                    <Link to='/'>
                      <h5 className="mb-2 text-2xl font-bold tracking-wide text-gray-900 dark:text-white inline-block">{e.name}</h5>
                    </Link>
                    <p className='mb-2'>
                      {e.smallPara}
                    </p>
                    ...
                    <p className='mb-5'>
                      <span className='font-bold'>Duration:</span> {e.duration}
                    </p>
...
export default BusinessFreelancing